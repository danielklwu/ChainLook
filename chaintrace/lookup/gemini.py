...
EXAMPLE_RESPONSE = """\
{
    ...
    "description": "The DAC32031 is a high-speed, 16-bit digital-to-analog converter (DAC) from Texas Instruments. It features a sampling rate of up to 500 MSPS, low power consumption, and a wide supply voltage range.",
...
def build_prompt(query: str, aggregated_text: str) -> str:
    ...
    Args:
        ...
        aggregated_text: Combined text from all scraped pages (pre-filtered).
...
def classify(prompt: str, model: str = DEFAULT_MODEL) -> str:
    ...
    for attempt in range(_MAX_RETRIES):
        try:
            ...
            if not any(fragment in exc_lower for fragment in _RETRYABLE_FRAGMENTS):
                raise RuntimeError(f"Gemini API error: {exc}") from exc
            wait = _RETRY_BACKOFF[attempt]
            logger.warning(
                "Gemini overload (attempt %d/%d), retrying in %ds: %s",
